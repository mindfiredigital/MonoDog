...
export async function getWorkflowRunJobs(
  ...
  try {
    ...
    AppLogger.error(`Failed to get jobs for run ${runId}: ${error}`);
    throw error;
  }
}
...
export async function getJobLogs(
  ...
  AppLogger.info(
    `Fetching job logs from GitHub: owner=${owner}, repo=${repo}, jobId=${jobId}`
  );
    ...
    const request = https.request(
      ...
      response => {
        ...
        if (
          ...
          if (location) {
              ...
              redirectResponse => {
                ...
                redirectResponse.on('end', () => {
                  const rateLimit: RateLimitInfo = {
                    limit:
                      parseInt(
                        redirectResponse.headers['x-ratelimit-limit'] as string
                      ) || 5000,
                    remaining:
                      parseInt(
                    ...
                    reset:
                      parseInt(
                        redirectResponse.headers['x-ratelimit-reset'] as string
                      ) || 0,
                    used:
                      parseInt(
                        redirectResponse.headers['x-ratelimit-used'] as string
                      ) || 0,
                  };
                  ...
                  if (
                    ...
                    AppLogger.error(
                      `GitHub API error after redirect ${redirectResponse.statusCode}: ${redirectBody.substring(0, 200)}`
                    );
                    reject(
                      new Error(
                        `GitHub API error: ${redirectResponse.statusCode} - ${redirectBody}`
                      )
                    );
                  } else if (
                    !redirectBody ||
                    redirectBody.trim().length === 0
                  ) {
                    AppLogger.warn(
                      `Job logs are empty after redirect (status=${redirectResponse.statusCode})`
                    );
                    resolve({ logs: '', rateLimit });
                  } else {
                    AppLogger.info(
                      `Fetched ${redirectBody.length} characters of job logs from redirect`
                    );
            ...
            redirectRequest.on('error', error => {
              AppLogger.error(
                `Failed to follow redirect for job logs: ${error.message}`
              );
              reject(error);
            });
            redirectRequest.end();
        ...
        response.on('end', () => {
          try {
            AppLogger.debug(
              `Job logs response: status=${response.statusCode}, length=${body.length}`
            );
            if (response.statusCode && response.statusCode >= 400) {
              AppLogger.error(
                `GitHub API error ${response.statusCode}: ${body.substring(0, 200)}`
              );
              reject(
                new Error(`GitHub API error: ${response.statusCode} - ${body}`)
              );
              ...
              AppLogger.warn(
                `Job logs are empty (status=${response.statusCode})`
              );
              resolve({ logs: '', rateLimit });
            } else {
 ...
 * \u001b[36m##[endgroup]\u001b[0m
 */
export function parseJobLogs(rawLogs: string, job: WorkflowJob): StepLog[] {
  const steps: StepLog[] = [];
...
export async function triggerWorkflow(
  ...
  if (workflowIdentifier.includes('/')) {
    ...
    workflowIdentifier =
      workflowIdentifier.split('/').pop() || workflowIdentifier;
  }

  const path = GITHUB_ACTIONS.TRIGGER_WORKFLOW_ENDPOINT(
    request.owner,
    request.repo,
    workflowIdentifier
  );
  ...
  try {
    AppLogger.debug(
      `Triggering workflow: ${workflowIdentifier} (original: ${request.workflow}) on ${request.owner}/${request.repo} branch: ${request.ref}`
    );
    ...
    AppLogger.error(
      `Failed to trigger workflow ${workflowIdentifier}: ${error}`
    );
    return {
      response: {
        success: false,
