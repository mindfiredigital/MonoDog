...
export const VALIDATION_ERRORS = {
  ...
  WORKING_TREE_NOT_CLEAN:
    'Please commit or stash all changes before publishing',
...
export const FILE_OPERATION_ERRORS = {
  ...
  FILE_NOT_WRITABLE: (filePath: string) =>
    `File is not writable or does not exist: ${filePath}`,
  PACKAGE_DIRECTORY_NOT_FOUND: (packagePath: string) =>
    `Package directory not found: ${packagePath}`,
  PACKAGE_JSON_NOT_FOUND: (packagePath: string) =>
    `package.json not found in directory: ${packagePath}`,
} as const;
