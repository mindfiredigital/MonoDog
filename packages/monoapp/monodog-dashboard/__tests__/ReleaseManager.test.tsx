...
const MockPackageSelector = ({ packages = [], selectedPackages = [], onSelectionChange = () => {}, onNext = () => {} }: any) => (
  <div data-testid="package-selector">
    <h2>Select Packages</h2>
    {packages.map((pkg: any) => (
      <div key={pkg.name}>
        <label>{pkg.name} v{pkg.version}</label>
      </div>
    ))}
...
const MockVersionBumpSelector = ({ packages = [], onConfirm = () => {}, onBack = () => {} }: any) => (
  <div data-testid="version-bump-selector">
    <h2>Version Bump Selection</h2>
    {packages.map((pkg: any) => (
      <div key={pkg.name}>
        <span>{pkg.name}</span>
        <button onClick={() => onConfirm({ [pkg.name]: 'minor' })}>Minor</button>
      </div>
    ))}
...
const MockChangesetPreview = ({ packages = [], onConfirm = () => {}, onBack = () => {} }: any) => (
...
const MockPublishConfirmation = ({ packages = [], summary = '', onReset = () => {} }: any) => (
...
describe('Release Manager', () => {
  ...
  describe('Changeset Preview', () => {
    ...
    test('validates summary length before confirming', () => {
      ...
      const textarea = screen.getByPlaceholderText('Describe changes') as HTMLTextAreaElement;
      ...
      fireEvent.change(textarea, { target: { value: 'This is a valid release summary' } });
      expect(mockConfirm).toHaveBeenCalledWith('This is a valid release summary');
    });
