...
export default function ReleaseValidation({
  ...
  const checks = [
    ...
    {
      name: 'Version Available on npm',
      passed: validation.checks.versionAvailable,
    },
  ];
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div
          className={`p-4 rounded-lg ${validation.isValid ? 'bg-green-50' : 'bg-red-50'}`}
        >
          <p
            className={`font-medium ${validation.isValid ? 'text-green-900' : 'text-red-900'}`}
          >
            {validation.isValid ? 'Ready to Release' : 'Not Ready'}
          </p>
          <p
            className={`text-sm mt-1 ${validation.isValid ? 'text-green-700' : 'text-red-700'}`}
          >
            {validation.isValid
