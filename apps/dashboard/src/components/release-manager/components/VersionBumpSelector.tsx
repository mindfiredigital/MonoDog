...
export default function VersionBumpSelector({
  ...
  const [bumps, setBumps] = useState<
    Record<string, 'major' | 'minor' | 'patch'>
    ...
    <div className="bg-white rounded-lg shadow p-6 space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">
          Step 2: Select Version Bumps
        </h2>
        <p className="text-gray-600">
          Choose the version bump type for each package. This determines the new
          version number.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {(Object.entries(bumpDescriptions) as [string, string][]).map(
          ([type, desc]) => (
